...
import { LikeThreadUseCase } from './like-thread'
...
let sut: LikeThreadUseCase
let user: User
let thread: Thread

describe('Like Thread', () => {
  beforeEach(async () => {
  ...
  it('should be able to like a thread', async () => {
    await sut.execute({
      userId: user.id,
      threadId: thread.id,
    })
    ...
    expect(isThreadLiked?.threadId).toEqual(thread.id)
    expect(isThreadLiked?.userId).toEqual(user.id)
  })

  it('should not be able to like a thread with empty userId or threadId', async () => {
    user.id = ''
    thread.id = ''
    await expect(() =>
      sut.execute({
  ...
  it('should not be able to like a thread with a non existent user', async () => {
    await expect(() =>
      sut.execute({
  ...
  it('should not be able to like a non existent thread', async () => {
    await expect(() =>
      sut.execute({
        userId: user.id,
  ...
  it('should increase likesCounter after a like', async () => {
    await sut.execute({
      threadId: thread.id,
      userId: user.id,
    })
